...
const CatalogExplorer: React.FC<CatalogExplorerProps> = ({ onOpenCatalog }) => {
  const catalogs = [
    { id: '1', title: 'Общий каталог 2025', label: 'MEBELSON 2025', type: 'PDF', color: 'bg-mebelson-red' },
    { id: '2', title: 'Кухонные системы', label: 'KITCHEN 2024', type: 'PDF', color: 'bg-[#333333]' },
    { id: '3', title: 'Детская мебель', label: 'KIDS 2024', type: 'PDF', color: 'bg-[#666666]' },
    { id: '4', title: 'Спальни и Гостиные', label: 'HOME 2024', type: 'PDF', color: 'bg-[#888888]' },
  ];
        ...
        <div key={catalog.id} className="bg-white rounded-2xl shadow-sm border border-mebelson-border overflow-hidden flex flex-col transition-all hover:shadow-2xl group">
          <div className={`${catalog.color} aspect-[3/4] relative flex items-center justify-center p-8 overflow-hidden`}>
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
               <button onClick={onOpenCatalog} className="bg-white text-mebelson-red p-4 rounded-full shadow-xl transform scale-75 group-hover:scale-100 transition-transform">
          ...
          <div className="p-6">
            ...
            <div className="space-y-2">
              <button onClick={onOpenCatalog} className="w-full bg-mebelson-dark text-white py-3 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-mebelson-red transition-all">
                Смотреть
              </button>
